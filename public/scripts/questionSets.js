...
function validateForm() {
  ...
  distinctNames = distinctNames.filter(function(o) { return distinctNames[o] !== 'userId' && distinctNames[o] !== 'others' && distinctNames[o] !== 'startingTime'})
...
function validateExpForm() {
  ...
  let names = $('input').map(function(){ return $(this).attr('name') })
  let distinctNames = $.unique(names)
  distinctNames = distinctNames.filter(function(o) { return (distinctNames[o] !== 'userId' && distinctNames[o] !== 'trial' && distinctNames[o] !== 'others' && distinctNames[o] !== 'startingTime')})
  for (let qn of distinctNames) {
    if (qn === 'qn29') {
...
function validateFormSurvey6() {
  ...
  let names = $('input').map(function(){ return $(this).attr('name') })
  let distinctNames = $.unique(names)
  distinctNames = distinctNames.filter(function(o) { return (distinctNames[o] !== 'userId' && distinctNames[o] !== 'trial' && distinctNames[o] !== 'startingTime')})
  for (let qn of distinctNames) {
    let radioSet = document.getElementsByName(qn)
    let selected = false
    for (let radio of radioSet) {
